...
cheat = do
  GameState state <- get
  let allCoordinatesWithEmptySet = M.mapMaybe (\_ -> Just S.empty) state.items
  let allItems = S.unions $ M.values state.items
  put $ GameState state { items     = M.union allCoordinatesWithEmptySet state.items
                        , inventory = S.union state.inventory allItems }
  tell (L.singleton ("You successfully cheated"))
